...
class IBClient():
    ...
    def __init__(self, username: str, account: str,
                 enable_automatic_install_portal: bool = True,
                 ib_gateway_host: str = r"https://localhost",
                 ib_gateway_port: str = r"5000",
                 client_gateway_path: str = None) -> None:
        ...
        self._enable_automatic_install_portal = enable_automatic_install_portal
        self.client_portal_folder = None
        if enable_automatic_install_portal is True:
    ...
    def create_session(self) -> bool:
        ...
        if self._enable_automatic_install_portal is False:
            return True
    ...
    def _set_server(self) -> bool:
        ...
        if 'news' in self.data_news(conid='265598'):
            print(success)
            return True
        if server_account_content is not None and 'set' in server_update_content.keys() and server_update_content[
            'set'] == True:
            print(success)
            return True
        elif ('message' in server_update_content.keys()) and (
                server_update_content['message'] == 'Account already set'):
            print(success)
            return True
        else:
            print(failure)
    ...
    def _check_if_server_running(self, process_id: str) -> bool:
        if self._enable_automatic_install_portal:
            return True
    ...
    def _check_authentication_non_input(self) -> bool:
        ...
        elif auth_response.get('authenticated', 'Null') in (False, 'Null') and auth_response.get('connected',
                                                                                                 'Null') in (
                False, 'Null'):
    ...
    def connect(self, start_server: bool = True) -> bool:
        ...
        {}
        """.format('-' * 80, self.ib_gateway_path + "/sso/Login?forwardTo=22&RL=1&ip2loc=on", self.server_process,
                   '-' * 80)
              )
    ...
    def close_session(self) -> None:
        ...
        # kill the process.
        return_code = subprocess.call("TASKKILL /F /PID {} /T".format(self.server_process),
